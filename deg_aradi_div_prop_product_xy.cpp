...
void model_cipher(GRBModel &model, int rnd) {
    ...
    */
    ...
    /******** in the following, we update x to xy in order
           to compute degree of xy after r rounds  *******/
    vector<vector<GRBVar>> y_copy(2, vector<GRBVar>(32));
    add_copy_constraints(model, y[rnd], y_copy);

    x[rnd]  = add_and_constraints(model, x[rnd], y_copy[0]);
    y[rnd] = y_copy[1];
    model.update();
    /***** similarly the above part could be modified for the computation
        of degrees of other products: wy,xy,wx,wz,zx *******************/
...
void apply_constraints_and_optimize(GRBModel &model,
    ...
    for (int target=0; target <32; target++){
        ...
        model_copy.optimize();
        deg = round(model_copy.getObjective().getValue()) ;
        cout << "Target bit: " <<"xy["<< target <<"], deg: " << deg << endl;
    }
    cout<<"========================================"<<endl;
    for (int target=0; target <32; target++){
        GRBModel model_copy = GRBModel(model);
